...
<script>
  ...
  import { authorizationClient, roleClient, userDetailClient } from "@/rest/client";
  import { DataStatusEnum, ResponseStatusEnum } from "@/tools/constant";

  import { CoreUtils, Dates, StringUtils } from "jcdt";
  ...
  export default {
      ...
      return {
        ...
        table: {
          columns: ( () => {
            ...
            this.getOwnPermissions().then( permissions => {
              if ( !( permissions.contains( "user_write" ) || permissions.contains( "grant" ) ) ) {
                return;
              }
                    ...
                    funcs.push( h( "Button", {
                      ...
                      on: {
                        click: async () => {
                          ...
                          modal.isShowModal = true;
                          modal.authorityId = row.authorityId;

                          let roles = ( await roleClient.getRoles() ).data.result,
                            existRoles = ( await roleClient.getSpecificUserRoles( row.id ) ).data.result;
    ...
    async mounted() {
      ...
      this.security.write = !( await this.isForbidden( "user_write" ) );
    },
    methods: {
      ...
      async modifyUserDetail() {
        if ( await this.isForbidden( "user_write" ) ) {
          return;
        }
      ...
      async modifyUserRole() {
        if ( await this.isForbidden( "grant" ) ) {
          return;
        }

        let modal = this.roleModal,
          response = ( await authorizationClient.grantRolesTo( modal.authorityId, modal.existRoles ) ).data;
