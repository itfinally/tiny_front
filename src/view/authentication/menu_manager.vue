    ...
    <Row type="flex">
        ...
        <Row class="frame-scroll" style="width: 100%; height: 100%;">
            <div style="margin: 0 .3rem; padding: .5rem 0; border-bottom: 1px solid #dddee1;"
                 v-if="this.security.write">
                <Button type="primary" @click="saveMenuTree">
                    保存菜单目录
                </Button>
            </div>
            <div>
...
<script>
  ...
  import parent from "@/component/base_component";

  import { DataStatusEnum, ResponseStatusEnum } from "@/tools/constant";
  import { ArrayDeque, CoreUtils, HashMap, HashSet } from "jcdt";
  import { menuClient, menuRoleClient } from "@/rest/client";
  ...
  export default {
                ...
                h( "span", [
                  h( "span", {
                    ...
                    h( "Button", {
                      ...
                      style: {
                        display: this.security.grant ? "" : "none",
                        marginRight: "0",
                        width: "4rem"
                      },
    ...
    async mounted() {
      ...
      this.translateTo( tree, menuResponse.data.result );
      this.tree.root[ 0 ].children = tree;

      this.url = this.getThisRouterPath();

      this.security.write = !( await this.isForbidden( "menu_write" ) );
      this.security.grant = !( await this.isForbidden( "grant" ) );
    },
    methods: {
        ...
        return h( "span", {
          ...
          h( "span", {
            ...
            h( "Button", {
              ...
              style: { display: this.security.grant ? "" : "none" },
      ...
      appendNode( root, node, data, isRoot ) {
        if ( data.disabled ) {
          ...
          // 如果当前节点已经删除但还没保存到服务器, 从 removes 集合中移除该元素
          // 否则加入到 recover 集合
          ...
          while ( pNode.disabled ) {
            if ( removes.contains( pNode.id ) ) {
              removes.remove( pNode.id );
            }
      ...
      removeNode( root, node, data ) {
        ...
        let hasChild = node => "children" in node && node.children.length > 0,
          removes = this.tree.removes,
          removeNode = data => {
            removes.add( data.id );
      ...
      async selectNode( root, node, data ) {
        ...
        let availableRoleResponse = menuRoleClient.queryAvailableRole( data.id ),
          ...
          availableRole = ( await availableRoleResponse ).data.result,
          existRole = ( await existRolesResponse ).data.result,
          menu = this.menu;
      ...
      async saveMenuTree() {
        if ( await this.isForbidden( "menu_write", true ) ) {
          return;
        }
        ...
        while ( !details.isEmpty() ) {
          ...
          if ( response.data.statusCode === ResponseStatusEnum.SUCCESS.statusCode ) {
            nameMapping.put( detail.name, response.data.result.id );
          }
        }
        ...
        this.translateTo( container, response.data.result );
        this.tree.root[ 0 ].children = container;

        this.$Message.info( { "content": "菜单已更新" } );
      },
      async saveMenuItem() {
        if ( ( await this.isForbidden( "grant", true ) ) || !this.isModified() ) {
          return;
        }
        ...
        if ( theNews.length > 0 ) {
          ...
          if ( theNewsRequest.data.statusCode !== ResponseStatusEnum.SUCCESS.statusCode ) {
            this.$Message.warning( { "content": `菜单权限更新失败 -> ${theNewsRequest.data.message}` } );
            return;
          }
        }
        ...
        if ( deletes.length > 0 ) {
          ...
          if ( deletesRequest.data.statusCode !== ResponseStatusEnum.SUCCESS.statusCode ) {
            this.$Message.warning( { "content": `菜单权限更新失败 -> ${deletesRequest.data.message}` } );
            return;
          }
        }
        ...
        if ( selected.name !== menu.name || selected.path !== menu.path ) {
          ...
          if ( updateMenuRequest.data.statusCode !== ResponseStatusEnum.SUCCESS.statusCode ) {
            this.$Message.warning( { "content": `菜单信息更新失败 -> ${updateMenuRequest.data.message}` } );
            return;
          }
        }
